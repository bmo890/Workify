...
export default function Homepage(props) {
	...
	return (
		<div>
			{/* Search Bar */}
			<div
				style={{ display: 'flex', justifyContent: 'center', padding: '5rem 10rem 0 10rem' }}
				className="container"
			>
				<div style={{ display: 'flex', justifyContent: 'center' }} className="container">
					<input
						...
						onChange={(event) => handleSearchParams(event.target.value)}
					/>
					<button
			...
			<div hidden={!noneFound}>No Jobs Found. Try A Different Search</div>
			<div>
				{searchResults && (
			...
			<div>
				{localJobs && (
					<div hidden={localJobs.length < 1}>
						<div style={{ fontSize: '2rem', margin: '2rem 0 .5rem 8rem' }}>
							{' '}
							All Jobs In {auth.user.location}
						</div>
						<div
							style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', fontSize: '1rem' }}
							className="container"
						>
							{localJobs.map((job) => {
								return (
